...
describe("Testing application end points: \n", () => {
  ...
  it(`should get end point /api/user/account/orders/:id with status code 401 and error message`, async () => {
    const response = await route.get(`/api/user/account/orders/${orderId}`);
    expect(response.status).toBe(401);
    expect(response.body.message).toEqual("Access denied, Faild to authenticate !");
  });
  ...
  it(`should get end point /api/user/account/orders/:id with status code 401 and error message`, async () => {
    const response = await route.delete(`/api/user/account/orders/${orderId}`);
    expect(response.status).toBe(401);
    expect(response.body.message).toEqual("Access denied, Faild to authenticate !");
  });
  ...
  it(`should get end point /api/user/account/ordered-products/:id with status code 401 and error message`, async () => {
    const response = await route.get(`/api/user/account/ordered-products/${opId}`);
    expect(response.status).toBe(401);
    expect(response.body.message).toEqual("Access denied, Faild to authenticate !");
  });
  ...
  it(`should get end point /api/user/account/orders/:id/products with status code 401 and error message`, async () => {
    const response = await route.post(`/api/user/account/orders/${orderId}/products`);
    expect(response.status).toBe(401);
    expect(response.body.message).toEqual("Access denied, Faild to authenticate !");
  });

  it(`should get end point /api/user/account/ordered-products/:id with status code 401 and error message`, async () => {
    const response = await route.delete(`/api/user/account/ordered-products/${opId}`);
    expect(response.status).toBe(401);
    expect(response.body.message).toEqual("Access denied, Faild to authenticate !");
  });
});
...
describe("Testing application routes functionalty: \n", () => {
  ...
  it(`should create order and get end point /api/user/account/orders and create new order for user ${userId}`, async () => {
    const response = await route
      ...
      .send({ userId: userId });
    orderTime = response.body.created_at;
  ...
  it(`should get end point /api/user/account/orders/:id/products and add product to order number ${orderId}`, async () => {
    const response = await route
      .post(`/api/user/account/orders/${orderId}/products`)
      .set("Content-type", "application/json")
      .set("Authorization", `Bearer ${userToken}`)
      .send({ _id: pId, quantity: schema.quantity });
    const { created_at } = response.body;
    opId = response.body._id;
    time = JSON.stringify(created_at).replaceAll(`"`, "");
    expect(response.status).toBe(201);
    expect(response.body.message).toEqual(
      `Product has been added successfully to order number (${orderId})`
    );
    expect(response.body).toEqual({
      _id: opId,
      order_id: orderId,
  ...
  it(`should not add product and get end point /api/user/account/orders/:id/products with status code 500 and error message`, async () => {
    const response = await route
      ...
      .set("Content-type", "application/json")
      .set("Authorization", `Bearer ${userToken}`)
      .send({ _id: "123", quantity: schema.quantity });
    expect(response.status).toBe(500);
    expect(response.body.message).toEqual(
  ...
  it("should update user and SET password to = (abc)", async () => {
    ...
    expect(result.body).toEqual({
      ...
      data: { _id: userId, email: schema.email, name: schema.name },
    });
  });
  ...
  it(`should get order number ${orderId}`, async () => {
    const result = await route
      .get(`/api/user/account/orders/${orderId}`)
      .set("Authorization", `Bearer ${userToken}`);
    expect(result.status).toBe(200);
    expect(result.body).toEqual({
  ...
  it(`should not update order status to it's current status with code 500`, async () => {
    const result = await route
      ...
      .set("Authorization", `Bearer ${userToken}`)
      .set("Content-type", "application/json")
      .send({ oid: orderId, status: schema.order_status });
    expect(result.status).toBe(500);
    expect(result.body).toEqual({
      message: `Error: Order number (${orderId}) already has a status of (${schema.order_status}) !`,
    });
  });
  ...
  it(`should not update order status to anything but (complete or active)`, async () => {
    const result = await route
      ...
      .set("Authorization", `Bearer ${userToken}`)
      .set("Content-type", "application/json")
      .send({ oid: orderId, status: "anything" });
    expect(result.status).toBe(500);
    expect(result.body.message).toEqual(
      `Unable to update orders with id (${orderId}) - Please enter value between [ active | complete ] for order status !`
    );
  });
  ...
  it("should update order status to (complete)", async () => {
    const result = await route
      ...
      .set("Authorization", `Bearer ${userToken}`)
      .set("Content-type", "application/json")
      .send({ oid: orderId, status: "complete" });
    expect(result.status).toBe(200);
    expect(result.body.message).toEqual("Order updated successfully");
    expect(result.body.order_id).toEqual(orderId);
    expect(result.body.order_status).toEqual("complete");
    expect(result.body.user_id).toEqual(userId);
    expect(result.body.created_at).toEqual(orderTime);
  });
  ...
  it("should not update order status to any other status if it's already (complete) with code 500", async () => {
    ...
    expect(result.status).toBe(500);
    expect(result.body).toEqual({
      message: `Error: Order number (${orderId}) already has a status of (complete) - you may review your order or delete it if you want !`,
    });
  });
  ...
  it(`should not delete order because of foregin key constrain`, async () => {
    const response = await route
      .delete(`/api/user/account/orders/${orderId}`)
      .set("Authorization", `Bearer ${userToken}`);
    expect(response.status).toBe(500);
    expect(response.body.message).toEqual(
      `Unable to delete order with id (${orderId}) - Please remove any products related to this order first !`
    );
  });

  // Ordered_Products

  it(`should not add products to order number ${opId}`, async () => {
    const result = await route
      .post(`/api/user/account/orders/${orderId}/products`)
      .set("Authorization", `Bearer ${userToken}`)
      .set("Content-type", "application/json")
      .send({ _id: pId, quantity: 10 });
    expect(result.status).toBe(500);
    expect(result.body).toEqual({
      message: `Error: Unable to add products to order number (${orderId}) because order status is already (complete)`,
    });
  });
  ...
  it("should get one ordered products", async () => {
    const result = await route
      .get(`/api/user/account/ordered-products/${opId}`)
      .set("Authorization", `Bearer ${userToken}`);
    expect(result.status).toBe(200);
    expect(result.body).toEqual({
      message: "Data generated successfully",
  ...
  it(`should not update ordered product number ${opId}`, async () => {
    const result = await route
      .put(`/api/user/account/ordered-products`)
      .set("Authorization", `Bearer ${userToken}`)
      .set("Content-type", "application/json")
      .send({ _id: pId, quantity: 0 });
    expect(result.status).toBe(400);
    expect(result.body).toEqual({
  ...
  it(`should not update ordered product number ${opId}`, async () => {
    const result = await route
      ...
      .set("Authorization", `Bearer ${userToken}`)
      .set("Content-type", "application/json")
      .send({ _id: "325b12e8-9676-4af8-8c12-34816ecb8ce1", quantity: 20 });
    expect(result.status).toBe(404);
    expect(result.body).toEqual({
      message: "Update failed !",
  ...
  it(`should update quantity of ordered product number ${opId} to (20)`, async () => {
    const result = await route
      .put(`/api/user/account/ordered-products`)
      .set("Authorization", `Bearer ${userToken}`)
      .set("Content-type", "application/json")
      .send({ _id: pId, quantity: 20 });
    expect(result.status).toBe(200);
    expect(result.body).toEqual({
  ...
  it(`should get end point /api/users/:uid/orders/:oid/account/review/ordered-products `, async () => {
    const response = await route
      .get(`/api/users/${userId}/orders/${orderId}/account/review/ordered-products`)
      .set("Authorization", `Bearer ${userToken}`);
    expect(response.status).toBe(200);
  });
  ...
  it("should delete one row from ordered products", async () => {
    const result = await route
      .delete(`/api/user/account/ordered-products/${opId}`)
      .set("Authorization", `Bearer ${userToken}`);
    expect(result.status).toBe(200);
    expect(result.body.message).toEqual(`Row number ${opId} was deleted successfully`);
  });
  ...
  it(`should delete order number ${orderId}`, async () => {
    const response = await route
      .delete(`/api/user/account/orders/${orderId}`)
      .set("Authorization", `Bearer ${userToken}`);
    expect(response.status).toBe(200);
